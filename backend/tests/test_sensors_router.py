...
from app.routers.sensors import (
    get_garden_sensor_summary,
    ingest_sensor_data,
    ingest_sensor_data_batch,
    register_sensor,
)
...
def test_register_sensor_rejects_missing_bed(db_session, user, garden):
    with pytest.raises(HTTPException) as exc:
        register_sensor(
            SensorRegister(
                garden_id=garden.id, bed_id=999, name="Probe", sensor_kind="soil_moisture"
            ),
            db=db_session,
            current_user=user,
        )
...
def test_register_sensor_rejects_missing_garden(db_session, user):
    with pytest.raises(HTTPException) as exc:
        register_sensor(
            SensorRegister(garden_id=999, name="Probe", sensor_kind="soil_moisture"),
            db=db_session,
            current_user=user,
        )
...
def test_ingest_sensor_data_normalizes_naive_timestamp(db_session, sensor):
    created = ingest_sensor_data(
        SensorDataCreate(value=30.5, captured_at=datetime(2026, 4, 1, 8, 0, 0)),
        db=db_session,
        sensor=sensor,
    )
...
def test_ingest_sensor_data_batch_rejects_large_batch(db_session, sensor):
    payload = SensorDataBatchCreate(
        readings=[SensorDataCreate(value=float(index)) for index in range(501)]
    )
...
def test_ingest_sensor_data_batch_inserts_rows(db_session, sensor):
    result = ingest_sensor_data_batch(
        SensorDataBatchCreate(
            readings=[SensorDataCreate(value=10.0), SensorDataCreate(value=11.0)]
        ),
        db=db_session,
        sensor=sensor,
    )
