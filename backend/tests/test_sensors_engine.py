...
def test_compute_irrigation_suggestions_handles_missing_and_dry_data():
    missing = compute_irrigation_suggestions(
        {"soil_moisture_series": [], "soil_temperature_series": []}
    )
...
def test_build_sensor_summary_filters_series_and_reports_latest():
    ...
    summary = build_sensor_summary(
        garden_id=1, sensors=sensors, readings=readings, horizon_hours=48
    )
