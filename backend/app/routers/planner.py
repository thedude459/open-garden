...
from ..core.dependencies import get_owned_bed, get_owned_garden, get_owned_placement, get_owned_planting
from ..models import Bed, CropTemplate, Garden, Placement, Planting, Sensor, SensorReading, Task, User
...
def _validate_spacing(
    ...
    *,
    ...
    placement_query = db.query(Placement).filter(Placement.garden_id == garden.id, Placement.bed_id == bed.id)
...
def create_bed(payload: BedCreate, db: Session = Depends(get_db), garden: Garden = Depends(get_owned_garden)):
...
def update_bed_position(payload: BedPositionUpdate, db: Session = Depends(get_db), bed: Bed = Depends(get_owned_bed)):
...
def rotate_bed_in_yard(db: Session = Depends(get_db), bed: Bed = Depends(get_owned_bed)):
    ...
    for other in other_beds:
        ...
        if intersects:
            raise HTTPException(status_code=409, detail="Bed cannot rotate because it would overlap another bed in the yard.")
    ...
    for placement in placements:
        if placement.grid_x < 0 or placement.grid_y < 0 or placement.grid_x >= original_cols or placement.grid_y >= original_rows:
            raise HTTPException(status_code=409, detail="Bed cannot rotate because one or more placements are outside the current bed bounds.")
...
def delete_bed(db: Session = Depends(get_db), bed: Bed = Depends(get_owned_bed)):
    ...
    db.query(Sensor).filter(Sensor.bed_id == bed.id).update({Sensor.bed_id: None}, synchronize_session=False)
    db.delete(bed)
    db.commit()
...
def create_placement(payload: PlacementCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    _validate_spacing(db, garden=garden, bed=bed, crop_name=payload.crop_name, grid_x=payload.grid_x, grid_y=payload.grid_y)
...
def delete_placement(db: Session = Depends(get_db), placement: Placement = Depends(get_owned_placement)):
    db.delete(placement)
    db.commit()
    return {"status": "deleted"}
...
def create_planting(payload: PlantingCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    if not direct_sow:
        ...
        auto_tasks.append((_crop_task_title(crop, "Start seeds indoors", crop_variety), start_date, f"Fill trays with seed-starting mix. Sow 2 seeds per cell at 70–75 F{zone_note}. Thin to strongest seedling once germinated."))
        harden_date = t0 - timedelta(days=7)
        if harden_date > start_date:
            auto_tasks.append((_crop_task_title(crop, "Harden off seedlings", crop_variety), harden_date, f"Move seedlings outside in a sheltered spot for 2-3 hrs/day{zone_note}. Increase exposure over 7 days to avoid transplant shock."))
        auto_tasks.append((_crop_task_title(crop, "Transplant seedlings to bed", crop_variety), t0, f"Plant at soil level. Water in with dilute liquid fertiliser{zone_note}. Protect with row cover if frost is still possible."))
    else:
        auto_tasks.append((_crop_task_title(crop, "Direct sow seeds", crop_variety), t0, f"Sow seeds at the depth and spacing shown on the packet{zone_note}. Keep soil moist (not saturated) until emergence."))

    auto_tasks.append((_crop_task_title(crop, "Water and check establishment", crop_variety), t0 + timedelta(days=3), "Check soil 1 inch deep - water when dry. Consistent moisture is critical in the first two weeks."))
    if days_to_harvest > 10:
        auto_tasks.append((_crop_task_title(crop, "Water and thin seedlings", crop_variety), t0 + timedelta(days=10), "Thin direct-sown seedlings if crowded. Water at the base; avoid wetting foliage."))

    if days_to_harvest > 14:
        auto_tasks.append((_crop_task_title(crop, "Weed bed", crop_variety), t0 + timedelta(days=14), "Remove weeds before they compete for nutrients. Shallow hoe to avoid disturbing roots."))
        auto_tasks.append((_crop_task_title(crop, "Pest and disease check", crop_variety), t0 + timedelta(days=7), f"Inspect leaves top and underside for pests, discolouration or disease{zone_note}. Treat organically early - neem oil or insecticidal soap for soft-bodied insects."))

    if days_to_harvest > 20:
        auto_tasks.append((_crop_task_title(crop, "Fertilise", crop_variety), t0 + timedelta(days=20), "Apply balanced fertiliser (e.g. 10-10-10) or side-dress with compost. Avoid excess nitrogen once flowering begins."))

    if days_to_harvest > 40:
        auto_tasks.append((_crop_task_title(crop, "Mid-season weed and water check", crop_variety), t0 + timedelta(days=40), "Remove weeds; check mulch depth (2-3 in helps retain moisture and suppress weeds). Deep watering every few days is better than shallow daily watering."))
    if days_to_harvest > 45:
        auto_tasks.append((_crop_task_title(crop, "Mid-season pest inspection", crop_variety), t0 + timedelta(days=45), "Check for disease progression; remove and dispose of infected leaves. Apply a second round of fertiliser if growth looks slow."))

    if days_to_harvest > 70:
        auto_tasks.append((_crop_task_title(crop, "Late-season check", crop_variety), t0 + timedelta(days=70), f"Ensure adequate water as fruit/roots swell{zone_note}. Watch for late blight (wet weather). Begin preparing bed for next crop rotation."))

    harvest_note = f"Expected ~day {days_to_harvest}{zone_note}."
    if crop_notes:
        harvest_note += f" {crop_notes}"
    auto_tasks.append((_crop_task_title(crop, "Harvest window opens", crop_variety), t0 + timedelta(days=days_to_harvest), harvest_note))

    for title, due_on, notes in auto_tasks:
        db.add(Task(garden_id=payload.garden_id, planting_id=planting.id, title=title, due_on=due_on, notes=notes))
    db.commit()
...
def log_harvest(payload: PlantingHarvestUpdate, db: Session = Depends(get_db), planting: Planting = Depends(get_owned_planting)):
