...
TEST_CMD='python -m venv /tmp/venv \
  ...
  && echo "Running backend integration tests with randomized order seed: ${RANDOMLY_SEED}" \
  && /tmp/venv/bin/pytest backend/tests -q -m "integration" --randomly-seed="${RANDOMLY_SEED}" -o addopts="" --cov=app --cov-branch --cov-report=term-missing --cov-report=xml:backend/coverage-integration.xml --cov-fail-under=0'
