...
def test_build_unified_timeline_combines_and_counts_all_event_sources():
    ...
    result = build_unified_timeline(
        tasks=[
            SimpleNamespace(
                ...
                due_on=today,
                notes="Use the drip line.",
                is_done=False,
            ),
            SimpleNamespace(
                id=2, title="Clean tools", due_on=today + timedelta(days=1), notes="", is_done=True
            ),
        ],
        weather={
            "daily": {
        ...
        sensor_summary={
            "irrigation_suggestions": [
                {
                    "title": "Bed 1 dry",
                    "detail": "Moisture is below threshold.",
                    "status": "irrigate_now",
                },
            ]
        },
        coach_response={
            "suggested_actions": [
                {
                    "title": "Mulch tomatoes",
                    "detail": "Reduce moisture swings.",
                    "priority": "high",
    ...
    assert result["events"] == sorted(
        result["events"], key=lambda item: (item["event_date"], item["category"], item["title"])
    )
    assert any(
        event["title"] == "Frost watch" and event["severity"] == "high"
        for event in result["events"]
    )
    assert any(
        event["title"] == "Sensor alert: Bed 1 dry" and event["severity"] == "high"
        for event in result["events"]
    )
    ...
    coach_events = _coach_recommendation_events(
        {
            "suggested_actions": [
                {
                    "title": "Scout aphids",
                    "detail": "Check undersides of leaves.",
                    "priority": "medium",
                    "category": "pests",
                },
