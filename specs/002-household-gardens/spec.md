...
3. **Given** a user who already owns a garden named “Backyard”, **When** they try to create or rename another owned garden to “Backyard”, **Then** the change is rejected with `You already own a garden with that name`; they MAY still be a collaborator or viewer on someone else’s garden also named “Backyard”.
...
5. **Given** invalid site input (zone outside 1–13, last frost and first frost on the same calendar day, or last frost on or after first frost in the calendar year), **When** the user tries to save, **Then** the save is rejected with the matching `VALIDATION_ERROR` message from `contracts/rest-api.md` (zone range or last-frost-before-first-frost) and prior valid values remain.
...
2. **Given** an owner, **When** they invite an email that has no account, **Then** they see `That email does not have an account yet`, and no membership is created (no external identity provider or magic-link invite in this feature).
...
- Create with blank or whitespace-only name: rejected with `Garden name is required`; no garden is created.
...
- A membership change happens on another device while this device is offline: when back online and the list or detail is refreshed, membership is current; stale cached gardens the user was removed from are not listed, not openable, and not treated as still accessible for new actions.
...
- **FR-001**: Authenticated users MUST be able to create a garden with a required non-empty name (trimmed; at most 120 characters) and optional notes, becoming the garden’s owner. The name MUST be unique among gardens that user currently owns (trim; case-insensitive). The same name MAY appear on gardens the user does not own (joined as collaborator or viewer).
...
- **FR-005**: A garden MUST store a site profile consisting of hardiness zone and last-frost and first-frost dates. Zone, when set, MUST be a single hardiness zone in the same 1–13 model used by the plant catalog. Each frost date, when set, MUST be an annual month-and-day pair (month and day both present, or both unset; they repeat every year; they are not a single calendar year). Last frost and first frost MAY be omitted independently of each other. When both frost dates are set, last frost MUST be earlier in the calendar year than first frost (northern-hemisphere spring-then-fall). Same-day and reversed last/first pairs MUST be rejected.
...
- **FR-008**: Owners MUST be able to add a member by the email address of an existing user account, choosing collaborator or viewer. The invitee need not be signed in at invite time. Inviting an email with no account MUST fail with `NOT_FOUND` and `That email does not have an account yet`; no membership is created. This feature MUST NOT introduce a third-party sign-in or email-invitation identity provider.
...
- **FR-011**: Collaborator and viewer permissions MUST match the Access Control roles matrix below (canonical). Collaborators MUST NOT manage membership or delete. Viewers MUST NOT update garden fields or membership.
...
- **Garden**: A named household planning place. Has a name (required, trimmed, max 120 characters), optional notes, and a site profile (hardiness zone, last frost date, first frost date). Identity is stable while the garden exists so it can be reopened, shared, and used by later planning features. Display names are unique per owner among existing gardens, not globally. After confirmed delete, the garden is gone (no archived copy in this feature).
...
- After reconnect and a refresh, garden list/detail MUST reflect current membership (a user removed while offline MUST NOT keep acting as a member; stale cached detail MUST NOT stay actionable).

## Success Criteria *(mandatory)*

### Measurable Outcomes

- **SC-001**: *(Usability study — not a CI gate. Technical first-load target is <2s; see plan.md Performance Goals.)* In usability testing, at least 90% of signed-in participants can create a named garden and reopen it from their list in under 2 minutes on the first attempt.
- **SC-002**: *(Usability study — not a CI gate; same technical <2s first-load target as SC-001.)* At least 90% of participants can set hardiness zone and both frost dates on a garden and confirm the same values on reopen, in under 2 minutes.
...
- **SC-005**: Inviting an email with no account never creates a membership and shows `That email does not have an account yet`; inviting an existing member does not duplicate membership (verified on fixtures).
...
- **SC-007**: *(Usability study — not a CI gate.)* At least 85% of participants agree they can tell who owns the garden and whether they can edit it or only view it.
...
- Invite is immediate membership for an existing account (the invitee need not be signed in at that moment; no pending-invite inbox, expiry, or email-send requirement in v1). If the product later sends notification email, that is out of scope here.
...
- Notes are plain text for household context (soil, sun, landlord rules); they are not structured bed lists. Maximum length is 4000 characters. Garden names are at most 120 characters after trim.
